...
LIB_FOLDER := lib
# Map Android NDK architecture names to libsodium build output names
# Note: NDK r28+ removed armeabi (armv6) support
MY_ARCH_FOLDER := $(TARGET_ARCH)
ifeq ($(MY_ARCH_FOLDER),arm)
    # armeabi-v7a will be used instead
    MY_ARCH_FOLDER = armv7-a
endif
ifeq ($(MY_ARCH_FOLDER),arm64)
    # Note: libsodium outputs as armv8-a+crypto
    MY_ARCH_FOLDER = armv8-a+crypto
endif
...
include $(CLEAR_VARS)
LOCAL_MODULE := sodium
LOCAL_SRC_FILES := $(abspath $(LOCAL_PATH))/../libsodium/libsodium-android-$(MY_ARCH_FOLDER)/$(LIB_FOLDER)/libsodium.a
LOCAL_LDFLAGS  += -fPIC
...
LOCAL_C_INCLUDES += $(abspath $(LOCAL_PATH))/../libsodium/libsodium-android-$(MY_ARCH_FOLDER)/include $(abspath $(LOCAL_PATH))/../libsodium/libsodium-android-$(MY_ARCH_FOLDER)/include/sodium
